    ...
    def __init__(self, path, *, method, query=None, body=None, headers=None):
        ...
        if not inspect.isasyncgen(body):
            if body is None:
                ...
                msg = (f'request body should be bytes, str or async generator, '
...
class Client:
    ...
    def __data_request(
            self, path, *, method, query=None, data=None, headers=None):
        if headers:
    ...
    def get(self, path, *, query=None, headers=None):
        return self.request(
            path, method='GET', query=query, headers=headers)

    def delete(self, path, *, query=None, headers=None):
        return self.request(
            path, method='DELETE', query=query, headers=headers)

    def head(self, path, *, query=None, headers=None):
        return self.request(
            path, method='HEAD', query=query, headers=headers)

    def options(self, path, *, query=None, headers=None):
        return self.request(
            path, method='OPTIONS', query=query, headers=headers)
